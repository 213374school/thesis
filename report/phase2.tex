...
We exploited an inherent limitation in facial detection; that faces far away in the image was rarely detected. Due to the nature of our datasets, and we do not try to cover up in any way that this particular label is highly overfitted for this exact purpose, waranted that when multiple faces,
%
% Noter fra Kim: Hvad vil i sige med dette? Pas på blah blah!
%
on average 2 or more faces, was detected we would almost certainly be in a crowd or overlooking a crowd from within.\\
...
The method for detecting a person in focus is simple. It works much like the in crowd detection, with the extra constraint that the bounding box must be within a centrally placed area of the frame.
%
