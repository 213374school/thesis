...
% Flere optical flow artikler her
...
% Flere crowd detection artikler?
...
%
% Lauge: Måske skal vi lige skrive lidt mere om Haar cascade. Enten flytte noget op fra Facial Detection afsnittet eller uddybe en lille smule hvordan det fungerer.
%
...
%
% Lauge: Vi skal lige have læst/skrevet lidt mere om denne ^
%
\subsection{Method}
%
...
%
\subsubsection{The presence of people}
%
% Noter fra Kim: Yes, i det mindste nogle flere detaljer. DESCRIBE HAAR CASCADE CLASSIFIER IN MORE DETAIL.
%
...
% Lauge: Der skal skrives noget mere om denne metode, hvis den skal nævnes
...
%
\subsubsection{Police Blinker Lights classifier}\label{sec:police_detection}
%
...
% %
% % Noter fra Kim: Police vehicles with flasing light. (Omdøb classifieren) NOW CALLED Police Blinker Lights
% %
...
% %
% respectively $[\|e\|,\|f\|]$ is below a certain treshold (meaning that $a,b,c,d$ respectively $e,f$ are roughly the same length).
% %%%%%%%%%%
% \\REWRITE:\\
%%%%%%%%%%
...
%
Our Overview classifier attempts to detect footage with little camera movement and little internal movement within the frames. Furthermore, in order to avoid overlap with the Person in Focus footage, which also often has these charactetistics, we also require the overview footage to not focus on any person in particular. The overview classifier uses the Support Vector Magnitude metadata described in section \ref{sec:svmdata}, the Optical Flow metadata described in section \ref{sec:opticalflowdata}, as well as the actual labels classified by the Person in Focus classifier described in section \ref{sec:infocus}. First the Optical Flow data is smoothed over 60 frames (slightly less than 3 seconds) in order to remove noise. The other two sets of metadata have already been smoothed or de-noised during their computation.\\
First we make sure that the frame has not already been classified as containing a Person in Focus. Now, let $s$, $t$ and $u$ be the threshold that we use to regulate the classifier. We then look at the mean vector length of the optical flow. Since each frame contains nine seperate optical flow means (for different parts of the image), we start by looking at how many of these means are below the threshold $s$. In our case $s=2$ made sense, but different resolutions will require different values. If the number of bins passing the test is larger than $t$ (in our case $7$, meaning that almost the entire image must have very little movement within), we futher make sure that the shift vector magnitude is less than $u$ (again, $10$ made sense in our case, but it will be dependant on resolution).
%
...
%
\subsubsection{Vertical Oscillation classifier}
%
...
%
\subsubsection{Police Blinker Lights classifier}
\subsubsection{Person in Focus classifier}
\subsubsection{Overview classifier}
\subsubsection{Vertical Oscillation classifier}
\subsubsection{In-Crowd classifier}
\subsubsection{Day \& Night classifier}
%
