...
Based on the method by Girgensohn et al. our understanding is as follows: for each pair of subsequent frames, the latter frame is shifted around, in all four directions (up, down, left, right). %, over 6 iterations. 
The initial shift distance is 32 pixels (suggested by Girgensohn et al.), halved in each iteration. %In each iteration this distance is halved. 
...
As our dataset contain mostly \textit{good} frames we wish to award a classifier that detect the few \textit{bad} frames that \textit{are} present. We therfore attempt to maximise positive precision along with \textit{negative} recall.\\
